...
public class FinalFormParticleEffect extends AbstractGameEffect {
    ...
    private float jerkY = 0;
    private float jerkX = 0;
    ...
    public FinalFormParticleEffect() {
        ...
        this.color = FinalForm.getColor();

        int mult = (int) MathUtils.random(0.8f, 1.2f);
        ...
        this.x = hitBox.cX + (float) this.img.packedWidth / 2.0F +
                MathUtils.random(
                        (-hitBox.width / 2.0F - 30.0F * Settings.scale),
                         hitBox.width / 2.0F + 10.0F * Settings.scale );

        this.y = hitBox.cY + (float) this.img.packedHeight / 2.0F * Settings.scale +
                MathUtils.random(
                        -hitBox.height / 2.0F - 100.0F * Settings.scale,
                        hitBox.height / 2.0F - 100.0F * Settings.scale);

        this.jerkX = MathUtils.random(10f, 15f) * Settings.scale;
        this.jerkY = MathUtils.random(5f, 10f) * Settings.scale;

        if (this.x < (hitBox.cX/2f) + 250f * Settings.scale){
            this.jerkX *= -1;
        }

        if (this.y < (hitBox.cY/2f) + 200f * Settings.scale){
            this.jerkY *= -1;
        }
...
//        this.vW = MathUtils.random(0.875f, 1.125f);
    ...
    public void update() {
        ...
        this.deltaY += Gdx.graphics.getDeltaTime() * this.jerkY * Settings.scale;

        this.deltaX += Gdx.graphics.getDeltaTime() * this.jerkX * Settings.scale;
