...
public class MasterFormParticleEffect extends AbstractGameEffect {
    ...
    public MasterFormParticleEffect() {
        this.img = ImageMaster.GLOW_SPARK_2;

        this.duration = MathUtils.random(1F, 2F);
        this.maxDuration = this.lastDuration = this.duration;
        this.fadeDuration = this.duration / 3.0F;

        this.color = MasterForm.getColor();
...
//        this.vW = MathUtils.random(0.875f, 1.125f);
        ...
        this.distanceOffset = MathUtils.random(0, 90.0F);
        this.renderBehind = MathUtils.randomBoolean(0.8F + (this.scale - 0.5F));
        this.rotation = MathUtils.random(360.0F);
        this.animationSpeed = MathUtils.random(.5f, 1f);
    }

    float theta = MathUtils.random(10000) ;
    float r = MathUtils.random(1.001f, 1.01f) ;
    @Override
    public void update() {
        ...
        //I think to achieve the effect i want, i need to give the theta rate of change a rate of change of its own ("jerk"-theta)
        theta -= 10;

        this.deltaX = (float) (Math.pow(r,theta) * MathUtils.cosDeg(theta) * Settings.scale);

        this.deltaY = (float) (Math.pow(r,theta) * MathUtils.sinDeg(theta) * Settings.scale);




        //this.deltaRotation += Gdx.graphics.getDeltaTime() * 30f * Settings.scale;
